...
//!
//! 이 모듈은 아직 파일을 생성하거나 복사하지 않는다. 게스트 파일시스템에서 받은
//! 상대 경로를 호스트 파일시스템에 매핑하기 전에 예약 이름, 경로 길이, 링크 추적
//! 위험을 차단하는 검증 경계를 제공한다.
...
use super::{GuestFileEntry, GuestPath, IoOperation, VirtualDiskError};
...
impl HostPathPolicy {
    ...
    pub fn map_entry(&self, entry: &GuestFileEntry) -> Result<PathBuf, VirtualDiskError> {
        if entry
            .attributes
            .contains(super::GuestFileAttributes::REPARSE_POINT)
        {
            return Err(VirtualDiskError::UnsafePath {
    ...
    pub fn map_path(&self, path: &GuestPath) -> Result<PathBuf, VirtualDiskError> {
        ...
        if !candidate.starts_with(&self.destination_root) {
            return Err(VirtualDiskError::UnsafePath {
                ...
                detail: "대상 루트 밖으로 매핑될 수 없습니다".to_string(),
            });
        }
...
mod tests {
    use super::*;
    use crate::virtual_disk::{GuestFileAttributes, GuestFileKind};
