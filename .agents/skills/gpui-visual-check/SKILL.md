...
그대로 수행한다. 이 파일은 Codex 어댑터이며 아래 표면 사항만 덧붙인다.
...
Windows의 Codex는 `CLAUDE_LOCAL`이다. ChatGPT 데스크톱의 Computer Use(`sky.*`,
...
`.Codex/agents/ui-visual-reviewer.md`를 사용한다. 그 에이전트는 자체 세션을 열므로
