...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| VDE-012 | E2E | [x] | [x] | [ ] | [ ] | [x] | [x] | `cargo check -p gpui-convenience-tools --locked`; `cargo test --all-targets --all-features` (140 passed, 4 ignored); `virtual_disk_panel_renders_copy_progress_and_issue_summary`에서 오류 행 selector·직접 상태 전환을 확인하고, `virtual_disk_suppressed_issue_keys` 저장/재로드와 항목별 상세 로그·미억제 오류 토스트 게이트를 검증; Clippy exit 0(기존 경고 7건); 격리 release 기본 캡처 `target/visual-validation/captures/vde012-default-004932.png`, processCount=0·sessionCount=0; 하네스 `simulate_click`이 해당 Stateful 버튼 listener를 dispatch하지 않아 실제 버튼 클릭과 VDI 오류 카드 화면은 미확인, VDE-019에서 실제 이미지 E2E 후속 |
