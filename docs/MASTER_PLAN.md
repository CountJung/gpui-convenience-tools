...
1000×700·920×700·1280×700 캡처로 수행했다. 후속 표준 VDI 1200×1000 캡처
`vde014-continuation-root-1848-034843.png`에서 `$Extend`·`many_subdirs` 폴더와
숨김/시스템 파일 행을 확인했다. 폴더 진입 Click은 현재 데스크톱 포그라운드가 대상 창으로
바뀌지 않아 하네스가 안전 차단했으므로 성공으로 기록하지 않았고, 실제 폴더 진입·범위 선택과
외부 키보드는 후속으로 남긴다.
...
실제 캡처와 세션 정리 확인으로 수행했다. 후속 표준 VDI 자동 복사에서도
`vde015-continuation-autocopy-1850-035033.png`의 파일 17개·915.3KB·실패 1개 요약과
격리 대상의 `-Force` 재검사 17개·937,234바이트를 확인했다. 손상된 `many_subdirs`는 화면·로그의
실패 사유와 일치했다. 대용량 폴더 내부 중지 지점은 별도 후속으로 남긴다.
...
실제 VBoxManage로 실행 중 VM을 대조하는 검증은 환경 주입이 필요하므로 후속으로 남긴다.
미지원 파티션 이미지는 MBR 타입 `0x83`·NTFS 부트 시그니처가 없는 VDI export fixture를
만들어 최신 release 앱에서 오류 카드와 파티션 경고를 확인했으며, fixture 파서 단위 테스트로
NTFS 오인 방지도 고정했다. 캡처는 `vde017-unsupported-partition-release-fixed-033450.png`이고
검증 세션 종료 후 프로세스·세션 루트는 0개였다. 독립 Visual Reviewer는 별도 잔여다.
...
실제 사용자 VDI를 건드리지 않고도 릴리스 앱의 VirtualBox 패널·숨김/시스템 항목·복사
실패 표시·대상 폴더 쓰기 경계를 확인할 수 있도록 검증 전용 시드 경로를 추가했다.
`-SeedVdi`는 합성 NTFS VDI를 세션 루트에 복사하고, `-AutoCopyVdi`는 선택 항목을 세션 전용
...
VDE-017의 미지원 파티션 경계도 같은 격리 릴리스 경로에서 검증했다.
`exports_unsupported_partition_vdi_fixture_when_requested`가 생성한 MBR fixture를 앱에
...
### Phase O-21 — 판단·환경 보류 항목 정합성 게이트 완료 ✅

`TODO.md`의 판단·환경 확인 절에 `decision-task-ids` 메타데이터를 추가하고,
`scripts/Assert-ProjectDocs.ps1`가 해당 ID의 형식·중복·활성 TODO 존재 여부를 검사하도록
보강했다. 이 게이트는 사용자가 결정해야 하는 범위를 자동으로 완료 처리하지 않으며,
보류 사유와 필요한 환경을 TODO에 계속 남기는 경계를 보장한다.

`DOCS_VERIFIED active=26 matrix=26`과 문서 차이 검사를 통과했다. 제품 판단이 필요한
...
### Phase O-23 — 프로젝트 구조 크기 자동 게이트 완료 ✅

`scripts/Assert-ProjectStructure.ps1`를 추가해 `app/src/**/*.rs`의 파일 수·전체 줄 수·최대
파일·800~1,000줄 경고 수를 `PROJECT_MAP.md`의 최종 측정값과 자동 대조하도록 했다. 1,000줄
이상 파일은 즉시 실패하며, `Verify-Workspace.ps1`가 문서 게이트 다음에 이 검사를 실행한다.
완료 당시 측정값은 54개 파일·20,894줄·최대 834줄·경고 3개로 일치했으며, 이후 변경분은
`PROJECT_MAP.md`의 최신 측정값으로 다시 대조한다.
...
### Phase O-25 — VDI 대용량 복사 중지 세분화 완료 ✅

기존 VDI 복사는 선택 항목 사이에서만 원자 중지 플래그를 확인해 큰 파일 하나 또는 하위
폴더를 처리하는 동안 중지 요청이 지연될 수 있었다. 복사 엔진에 청크 전·후 취소 확인을
추가하고, 취소된 파일의 부분 결과를 제거한 뒤 상위 디렉터리·형제 항목 순회를 즉시
중단하도록 연결했다. 부분 파일 제거 실패는 별도의 I/O 사유로 반환해 조용히 남기지 않는다.

`cancellation_stops_between_chunks_and_removes_partial_file` 회귀 테스트로 두 번째 청크
경계에서 중지되고 부분 대상 파일이 남지 않는 것을 확인했다. 실제 대용량 VDI에서의
포그라운드 중지 E2E는 별도 환경 검증 항목으로 유지한다.

### Phase O-26 — 탐색기 Shift 범위 선택 보강 완료 ✅

탐색기 행 클릭은 기존에 Ctrl·Shift를 같은 additive 토글로 처리해 Shift 클릭의 연속
범위 선택이 동작하지 않았다. 현재 목록의 선택 기준점을 저장하고, Shift 클릭 시 기준점과
대상 사이를 순방향·역방향으로 선택한다. Ctrl+Shift는 기존 선택 집합에 범위를 추가하며,
폴더 이동·새로고침·파티션 전환 시 기준점을 초기화한다.

`shift_range_selection_uses_anchor_in_both_directions` 테스트로 양방향 범위와 기준점 소실
경계를 검증했다. 최신 release 1200×1000 캡처 `vde014-shift-range-release-final-045106.png`에서
16개 선택·숨김/시스템 행·카드 경계를 확인했지만, 실제 화면 마우스 Shift 입력은 포그라운드
...
### Phase O-29 — 실제 VDI 중지 경계 보조 검증 완료 ✅

대용량 파일 중지 E2E를 포그라운드 키 입력에 의존하지 않고 확인할 수 있도록 검증 전용
`-CancelAfterMs` 시드를 추가했다. `-AutoCopyVdi`와 함께 사용할 때 release 앱의 실제
복사 작업이 시작된 뒤 `stop_virtual_disk_copy` 경로를 호출한다. 일반 사용자 설정과 제품
기본 실행에는 영향을 주지 않는다.

종료된 실제 `TACS.vdi`에서 200ms 시나리오를 실행해 `중지됨 · 파일 0개 · 0 B` 요약과
1200×1000 캡처를 확인했다. 같은 VDI의 루트 및 `$Extend` 데이터는 1,000~1,500ms 안에
먼저 완료되어 청크 중간 취소까지는 재현되지 않았다. 따라서 이번 단계는 실제 release
중지 상태·세션 정리 경계의 보조 증거로 기록하며 VDE-015 전체 완료로 처리하지 않는다.

virtual-disk 관련 테스트 72개·무시 2개, release build, VM `poweroff`, 원본 VDI 길이
불변, 검증 종료 후 process/session 0을 확인했다. 실제 대용량 파일 청크 중지, 외부
키보드, 독립 Visual Reviewer는 VDE-015·VDE-016·VDE-019의 잔여 조건으로 유지한다.

### Phase O-30 — D-020·G-001 2차 자체 시각 교차 확인 기록 ✅

구현 세션과 다른 격리 release 세션을 열어 최근 동기화 이력과 자동 시작 패널을 다시
확인했다. D-020은 994×702에서 이력 카드의 세로 스크롤 경계를 유지했고, 1280×900에서
중지·실패 포함·성공 3행을 카드 내부에 표시했다. G-001은 두 창 크기에서 자동 시작 상태
카드·작업 제어·안내 문장이 폭 밖으로 밀리지 않았다.
...
### Phase O-32 — VDE-015·VDE-019 다중 파티션 실제 VDI 중지 재검증 ✅

검증 하네스의 VDI 시드가 항상 첫 번째 파티션만 선택해 실제 다중 파티션 디스크의 다른
파일시스템을 대상으로 확인하기 어려웠다. 검증 전용 `-VdiPartitionNumber`와 환경 변수 주입을
추가했으며, 기본값은 기존처럼 첫 번째 파티션을 유지한다. 종료된 실제 TACS VM에서 GPT
파티션 5번 선택과 파티션 1번 `$Extend` 복사를 확인했다. 250ms는 `중지됨 · 파일 0개 ·
0 B`, 600ms는 `완료 · 파일 8개 · 4.1 MB`였고, 원본 길이 불변·VM `poweroff`·프로세스와
세션 루트 0을 확인했다.

실제 대용량 파일의 청크 중간 중지는 여전히 재현되지 않아 VDE-015·VDE-019 완료 조건으로
승격하지 않았다.

### Phase O-33 — VDE-015 실제 대용량 파일 중지 경계 보조 검증 ✅

자연 속도의 실제 TACS VDI는 작은 시스템 파티션 복사가 빠르게 끝나 대용량 파일의 청크
중간 중지를 안정적으로 재현하기 어려웠다. 검증 전용 `-ReadDelayMs`를 추가해 일반 실행과
사용자 설정에는 영향을 주지 않고 게스트 `read_at` 호출 사이에만 지연을 삽입했다. 종료된
TACS의 `$Extend/$RmMetadata/$TxfLog`에서 2MiB 파일을 대상으로 `-ReadDelayMs 1000`과
`-CancelAfterMs 2200`을 사용한 결과, 실행 중 0바이트 부분 파일 생성과 취소 후 미완성
파일 제거를 관찰했고 UI 요약 `중지됨 · 파일 2개 · 64.0 KB`를 확인했다. 원본 길이
85,269,151,744바이트 불변, VM `poweroff`, 프로세스·세션 루트 0을 확인했다.

이는 복사 엔진의 청크 취소·부분 파일 정리를 실제 VDI와 연결한 보조 증거다. 자연 속도
대용량 청크 중지, 외부 릴리스 버튼 조작, 독립 Visual Reviewer는 사용자 환경 의존 항목으로
여전히 완료 처리하지 않는다.
...
#### D-006 — 사전 순회 비용 측정·방향 결정 완료 ✅

- 3,000개 파일 임시 원본을 3회 측정한 결과 사전 순회는 매번 `0.003초`, 실제 동기화는
  `3.262~3.308초`, 합계 기준 추가 비용은 약 `0.1%`였다.
- 따라서 정확한 전체 수·퍼센트 진행률을 위해 사전 순회를 채택하고, 실제 전체 수 전달은
  D-007, 진행률 UI는 D-008에서 구현한다. 현재 측정은 신규 파일 복사 기준이므로 이미
  동기화된 대규모 폴더에서는 별도 회귀 측정이 필요하다.
- 실제 종료 TACS `TACS.vdi`의 접근 가능한 NTFS 파티션 1은 파일 19개·폴더 9개였고,
  읽기 전용 트리 열거는 1회 `0.524초`, 2회 `0.127초`였다. 주 OS 파티션은 BitLocker라
  전체 OS 파일 기준의 사전 순회 자료로 사용하지 않았다.
...
#### D-011 — 실시간 파일 감시와 디바운스 완료 ✅

- `WatchManager`가 작업별 `notify` 재귀 watcher를 소유하고 파일 변경 이벤트를 작업 ID로만
  전달한다. 연속 변경은 마지막 변경 이후 기본 2초가 지나야 하나의 동기화 실행으로 예약한다.
- 작업 삭제·경로 변경·감시 방식 변경 때 watcher와 대기 상태를 정리하며, UI나 watcher callback에서
  파일 복사를 직접 수행하지 않고 기존 동기화 엔진을 재사용한다.
- 실제 임시 폴더의 파일 변경을 watcher가 수신하는 통합 테스트와 디바운스 단위 테스트를 추가했다.

#### D-012 — 감시 실패 시 주기 모드 강등 완료 ✅

- watcher 생성·경로 등록·callback 오류를 작업별로 한 번만 보고하고, 백그라운드 이벤트를 통해
  `WatchMode::Interval`로 안전하게 되돌린다.
- 강등 시 설정을 저장하고 WARN 로그와 경고 토스트를 남긴다. 같은 오류가 반복되어도 동일 알림을
  계속 표시하지 않으며, 감시 방식이나 작업 경로가 바뀌면 오류 상태를 초기화한다.

#### D-013 — 감시 방식 선택 UI 완료 ✅

- 파일 동기화 설정에 `실시간 감시` 토글을 추가하고, 실시간 모드에서는 주기 선택 UI를 숨긴다.
  작업 카드에는 `실시간` 상태를 표시하고 2초 quiet debounce 동작을 안내한다.
- GPUI 토글·강등 회귀 테스트와 필수 20개 화면 테스트를 통과했다. 격리 릴리스 캡처
  `target/visual-validation/captures/d011-realtime-panel-015924.png`에서 작업 카드·토글·안내
  문구를 확인했으며, 캡처 세션 종료 후 프로세스와 임시 세션 루트가 남지 않았다.

전체 151 passed·4 ignored, Clippy exit 0(기존 경고 7건), `git diff --check`를 통과했으며
구현 커밋은 `e68a3f6`이다.
