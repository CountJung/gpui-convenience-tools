# Development Guide — 공통 개발 지침 정본

이 문서는 모든 에이전트와 개발 도구가 공유하는 프로젝트 규칙의 단일 정본이다. 에이전트
어댑터(`AGENTS.md`, `CLAUDE.md`, `.github/copilot-instructions.md`)에는 이 문서를 복사하지
않고, 해당 에이전트가 자주 잊는 표면별 주의사항과 참조 경로만 둔다.

문서 소유권은 다음과 같다.

- 사용자 안내: `README.md`
- 단계 계획·완료 이력: `MASTER_PLAN.md`
- 구조·줄 수·공용 유틸: `PROJECT_MAP.md`
- 미완료 작업: `TODO.md`
- 공통 규칙: 이 문서
...
아키텍처, 화면 구성, 프로세스 관계, 데이터 흐름, 상태 전이처럼 **배치와 연결을 설명하는
표 형태의 그림**은 고정 폭 ASCII 상자나 공백 정렬 텍스트로 작성하지 않는다. 문서 렌더러와
화면 폭에 따라 일그러지기 쉬우므로 `mermaid` 코드 블록으로 작성한다.

```mermaid
flowchart LR
    source["원본"] --> process["처리"] --> target["대상"]
```

실제 값의 비교, 작업 체크리스트, 파일별 줄 수처럼 행·열 검색이 핵심인 **데이터 표**는
...
- **치명적이지 않은 판단은 묻지 말고 먼저 구현한다.** 표기·용어 통일, 패딩·간격, 기본값,
  중복 제거처럼 되돌리기 쉬운 결정은 합리적으로 정해 구현하고, 무엇을 어떻게 판단했는지
  결과 보고에 한 줄로 남긴다. 사용자가 화면을 보고 방향을 잡는 편이 빠르다.
  - **"화면이 바뀌므로 보류"는 사유가 아니다.** 같은 값을 화면마다 다르게 찍고 있으면
    그 자체가 결함이다(예: 같은 주기를 `60초`와 `1분`으로 나눠 표기). 바뀐다는 사실을
    적고 통일한다. 리팩터와 섞지 않는 것만 지키면 된다.
  - 여전히 먼저 확인할 것: 파일 삭제·덮어쓰기, 외부로 나가는 동작(태그 push·릴리즈),
    되돌리기 어려운 config 마이그레이션, 선택에 따라 작업 전체가 달라지는 갈림길.
- MasterPlan 단계 기준으로 작업 범위를 결정한다.
- 단계 목표를 벗어나는 아키텍처 변경은 사용자 요청이 없는 한 수행하지 않는다.
- 단계를 끝내면 `MASTER_PLAN.md`「구현 완료 단계」에 결과를 적고, 그 단계에서 소화한 항목을
  `TODO.md`에서 지운다. 완료 이력은 `MASTER_PLAN.md`, 미착수 대기열은 `TODO.md`가 정본이다.
...
4. 패널의 조작 흐름에 맞춰 다음 두 레이아웃 중 하나를 선택한다.
   - **독립 탐색형**: 기능 목록과 설정을 동시에 비교해야 하면 스플리터(`h_resizable`)로
     나누고 각 영역을 `window::scroll_pane`으로 감싼다.
   - **연속 작업형**: 작업 선택 → 설정 → 실행 → 결과 확인이 한 흐름이면 별도 설정
     사이드바를 만들지 않고 전체 너비의 단일 `scroll_pane`에 세로로 배치한다.
   - 선택·설정·실행 값이 서로 의존하는데 좁은 독립 pane으로 나누지 않는다.
5. 앱 전역 설정(테마, 로그 보관)만 `window/settings.rs`에 둔다.
   기능별 설정을 전역 설정 페이지에 넣지 않는다.
6. 자체 스플리터 또는 단일 스크롤 뷰포트를 쓰는 패널은 `app.rs`의 `fills_height` 목록에
   추가한다.
...
| 줄 수 | 조치 |
| --- | --- |
| ~800줄 | 주의. `PROJECT_MAP.md`에 리팩터링 계획(중복 후보 포함)을 미리 적어 둔다 |
...
`PROJECT_MAP.md`에 적는다.** 판단 근거 없이 미루지 않는다.
...
- 판정 기준:
  ...
  | 상황 | 조치 |
  | --- | --- |
  | 같은 책임이 2개 파일에 존재 | 승격 후보. `PROJECT_MAP.md`에 기록 |
...
`PROJECT_MAP.md`는 저장소의 **구조·크기·공용 유틸을 추적하는 단일 문서**다.

- 소스 파일을 **추가·삭제·분할**했거나 **헬퍼를 공용으로 승격**했으면 같은 작업에서
  `PROJECT_MAP.md`를 갱신한다.
...
- `PROJECT_MAP.md`, `TODO.md` 같은 현재 상태·대기열 문서에서 처리 완료 항목을 취소선으로
  남기지 않고 행이나 항목 자체를 삭제한다. 보존할 가치가 있는 완료 결과는 `MASTER_PLAN.md`
  「구현 완료 단계」 또는 `PROJECT_MAP.md`「리팩터링 이력」에 한 번만 기록한다.

## TODO 작업 ID 및 진행 추적 기준

- `TODO.md`의 모든 미착수 작업에는 영역별 접두사와 순번으로 만든 **고유 작업 ID**를 붙인다.
  예: `VDE-001`(Virtual Disk Explorer), `D-001`(파일 동기화), `T-001`(테스트).
- 각 작업은 `- [ ] ID | 0% | 작업 내용` 형식으로 기록한다. 구현 중에는 체크박스와 진행률을
  함께 갱신하고, 완료한 항목은 `TODO.md`에서 삭제한 뒤 `MASTER_PLAN.md`와
  `PROJECT_MAP.md`에 완료 이력을 남긴다.
- 하위 작업도 독립적으로 확인할 수 있도록 각각 ID를 부여한다. 상위 작업의 체크만으로
  하위 작업이 완료된 것으로 간주하지 않는다.
- 작업 ID는 한 번 사용한 뒤 재사용하지 않는다. 작업이 취소되거나 범위가 바뀌면 기존 ID를
  삭제하지 말고 `MASTER_PLAN.md`의 결정·이력에 남긴 뒤 새 ID를 발급한다.
- 기능 구현 전에 작업 목록을 먼저 갱신하고, 작업 중에는 실제 코드·테스트·시각 검증
  증거에 맞춰서만 진행률을 올린다. 추정 진행률이나 빌드만 통과한 진행률은 기록하지 않는다.
- 진행 중 작업의 설명은 `남은 일`을 첫 문장으로 두고, 사용자의 결정·필요한 환경·다음 검증
  행동만 적는다. 완료한 구현·테스트·캡처의 긴 이력은 `VERIFICATION.md`와
  `MASTER_PLAN.md`에 기록하고 활성 TODO 항목에는 반복하지 않는다.
- 사용자·제품 판단 또는 실행 환경이 필요한 보류 항목은 `TODO.md`의
  `판단·환경 확인이 필요한 보류 항목` 절에 따로 기록한다. 절의
  `decision-task-ids` 메타데이터는 활성 TODO ID만 가리켜야 하며,
  `scripts/Assert-ProjectDocs.ps1`가 중복·고아·잘못된 ID를 검사한다.
...
- 스플리터의 각 영역은 `window::scroll_pane`으로 감싸고, 표시 영역을 넘을 때 세로 스크롤과
  스크롤바가 동작하도록 한다.
- 연속 작업형 패널은 페이지 전체를 하나의 `window::scroll_pane`으로 감싸며, 내부 섹션들이
  동일한 가용 너비를 사용해야 한다.
...
- **`scroll_pane`은 가로를 `overflow_x_hidden`으로 잠근다.** 세로만 스크롤로 열어 두면
  가로가 `visible`로 남아, 컨텐츠가 뷰포트 폭이 아니라 자기 내용 폭으로 잡히는 상황이
  생긴다. 그러면 카드마다 오른쪽 끝이 달라지고 페이지가 가로로 삐져나간다.
- 너비 회귀 테스트는 **컨텐츠가 뷰포트보다 좁아지는 넓은 창까지** 확인한다. 좁은 창에서는
  컨텐츠가 어차피 뷰포트 폭으로 잘려 증상이 드러나지 않으므로, 최소 지원 폭만 검증하면
  이 회귀를 놓친다.

### 카드·컨테이너 경계 규칙

- `content-area`, 스플리터 pane, `scroll_pane`의 컨텐츠 루트는 `min_w_0`와 가로
  `overflow_x_hidden`을 함께 적용해 자식의 최소 콘텐츠 폭이 카드 밖으로 확장되지 않게 한다.
- 카드와 카드 내부의 가로 행은 `w_full`·`min_w_0`를 기본으로 사용한다. 고정 폭이 필요한
  버튼·토글·삭제 열에는 `flex_shrink_0`를 주고, 이름·경로·설명처럼 길이가 변하는 텍스트에는
  `min_w_0`와 `overflow_hidden`을 준다.
- 한 줄로 유지해야 하는 식별자·버튼은 `whitespace_nowrap`을 적용한다. 좁은 pane에서
  핵심 정보가 세로로 밀리는 경우에는 행을 2줄 메타데이터 구조로 바꾸어 카드 높이 안에서
  표시하고, 고정 열이 내용을 밀어내도록 두지 않는다.
- 이 경계 규칙은 패널별 임시 margin보다 우선한다. margin/padding은 시각적 여백에만
  사용하고, 넘침 방지는 컨테이너 제약과 flex 최소 폭으로 해결한다.

### 리스트·그리드 컬럼 폭 규칙

- 컬럼은 정보 가치 순서로 폭을 배분한다. 파일명·경로처럼 사용자가 식별에 먼저 쓰는
  컬럼은 남은 폭을 우선 사용하고, 종류·속성·크기처럼 보조 정보는 읽을 수 있는 최소 폭을
  명시한다. 보조 컬럼의 고정 폭이 파일명 컬럼을 압박하도록 만들지 않는다.
- 헤더와 데이터 행은 같은 폭 계산을 공유한다. 고정 컬럼에는 `w`와 `min_w`,
  `flex_shrink_0`, `whitespace_nowrap`, `overflow_hidden`을 함께 적용해 텍스트가
  눌리거나 옆 컬럼으로 밀리지 않게 한다. 핵심 식별자 컬럼에는 합리적인 `min_w`를 둔다.
- 사용자가 스플리터나 컬럼 폭을 조정할 수 있게 만들었다면 해당 폭을 설정 스키마에 등록하고
  정상 범위로 보정한다. 조정 직후 저장하고 앱 재시작 후 복원되는 회귀 테스트를 추가한다.
  구버전 설정에는 `serde(default)`를 적용해 기존 파일을 깨지 않는다.
- 폭을 바꿀 수 있는 UI는 최소·기본·넓은 창에서 핵심 컬럼의 bounds와 텍스트 경계를
  확인한다. 자동 폭 계산만 믿지 말고, 고정 폭 합계와 핵심 컬럼 최소 폭이 스플리터의
  반대편 최소 폭을 넘지 않는지 테스트한다.
...
- 작업별 검증 순서·E2E 사용법·게이트 체크는 `VERIFICATION.md`를 정본으로 한다. 이 문서와
  TODO에 실행 절차를 복사하지 말고 작업 ID와 증거 식별자만 참조한다.
...
- 스위치·테마: light/dark와 변경 관련 테마에서 실제 스위치를 on/off 조작하고 상태 전이를
  ...
  대체하지 않는다.
- 파일 동기화: 최소 창 높이에서 단일 페이지를 wheel 입력으로 스크롤하고 offset 변화,
  작업 목록·설정·실패 기록의 동일 너비, 마지막 기록의 viewport 진입을 단언한다. 실행
  버튼은 현재 입력값 저장, 공유 작업 상태 갱신, 선택 작업 ID 큐 등록을 함께 단언한다.
...
- 계획과 완료 이력: `MASTER_PLAN.md` / 미착수 대기열: `TODO.md`
- 구조·크기·공용 유틸 추적: `PROJECT_MAP.md`
