...
| 2026-09-22 | `app/src/app/virtual_disk_ops.rs`·`app/src/window/virtual_disk.rs`·`app/src/app/validation.rs` | VDE-014 Shift 범위 선택 보강 | Shift 클릭이 Ctrl 클릭과 같은 단순 additive 토글로 처리되어 범위 선택이 동작하지 않음 | 목록 기준점(anchor)을 저장하고 순방향·역방향 Shift 범위 선택, Ctrl+Shift 범위 확장을 연결; `shift_range_selection_uses_anchor_in_both_directions` 테스트와 최신 release 1200×1000 캡처 `vde014-shift-range-release-final-045106.png` 추가 | 캡처에서는 16개 선택·숨김/시스템 행·카드 경계를 확인했지만 실제 화면 마우스 Shift 클릭은 포그라운드 안전 하네스 제약으로 별도 E2E 미검증 |
...
| 2026-09-22 | `app/src/sync/tests.rs`·`app/src/virtual_disk/vdi.rs`·`docs/TODO.md`·`docs/VERIFICATION.md`·`docs/MASTER_PLAN.md` | D-006 사전 순회 비용 측정·결정 | 전체 수 진행률을 위해 원본을 두 번 열거할지 비용 근거가 없음 | 3,000개 파일 3회 측정에서 사전 순회 `0.003초`, 실제 동기화 `3.262~3.308초`, 추가 비용 약 `0.1%`; 종료 TACS `TACS.vdi` NTFS 파티션 1은 파일 19개·폴더 9개, 트리 1회 `0.524초`·2회 `0.127초`; 정확한 진행률을 위해 사전 순회 채택, D-007·D-008 구현으로 이관 | 신규 파일 복사 기준 측정이며 대규모 무변경 폴더 비용은 D-007·D-008 구현 후 별도 회귀 측정 |
| 2026-09-22 | `app/src/sync/mod.rs`·`app/src/app/background.rs`·`app/src/app/state.rs`·`app/src/app/events.rs`·`app/src/app/tests/file_sync.rs`·`docs/TODO.md`·`docs/VERIFICATION.md`·`docs/MASTER_PLAN.md` | D-007·D-008 사전 총량·진행률 바 | D-006에서 채택한 사전 순회 결과가 UI 이벤트와 진행률 표시까지 연결되지 않음 | 정책 일치 사전 순회·`SyncProgress.total`·백그라운드 이벤트·진행률 계산·하단 고정 Progress 바를 구현하고, 사전 수/실제 보고 단위 및 compact 상태 표시줄 bounds를 GPUI 회귀 테스트로 고정; 전체 173 passed·5 ignored·Clippy `-D warnings`·필수 GPUI 31개; 실제 release 캡처 `d008-progress-running-1000x700-205653.png` | 사전 순회 권한 오류 시 진행률을 숨기는 보수적 경계; 독립 Visual Reviewer는 후속 |
...
| 2026-09-22 | `scripts/Invoke-ClaudeVisualCheck.ps1`·`docs/PROJECT_MAP.md` | G-003 드래그 실패 정리 경계 보강 | 드래그 중 상대 마우스 이동이 실패하면 전역 마우스 왼쪽 버튼이 눌린 채 남을 수 있음 | `Drag` 예외 경로에서 `LEFTUP`을 보장하고 스크립트 구문·구조 게이트를 재검증; 대상 창 포그라운드 실패 시에는 입력을 보내지 않는 기존 안전 경계 유지 | 실제 데스크톱 입력과 독립 Visual Reviewer는 여전히 환경 의존 |
| 2026-09-22 | `scripts/Invoke-ClaudeVisualCheck.ps1`·`docs/PROJECT_MAP.md` | Click·Ctrl+A 입력 실패 정리 경계 보강 | 좌클릭 해제 또는 Ctrl 키 해제가 실패하면 전역 입력 상태가 남을 수 있음 | `Click`의 `LEFTUP`, `Ctrl+A`의 Ctrl 해제를 예외 경로에서 보장하고 구문·문서·전체 검증 게이트를 재실행; 기존 포그라운드 안전 차단 유지 | 실제 데스크톱 입력과 독립 Visual Reviewer는 여전히 환경 의존 |
| 2026-09-22 | `scripts/Invoke-ClaudeVisualCheck.ps1`·`docs/TODO.md`·`docs/VERIFICATION.md` | G-003 최신 release 드래그 재시도 | 현재 release의 divider 위치와 입력 경계를 다시 확인할 필요가 있음 | 1000×700 격리 release를 실행해 `next-g003-before-065645.png`를 캡처; `Drag`는 현재 포그라운드 `66246`으로 안전 차단되어 전역 입력을 보내지 않았고 세션 종료 후 프로세스·임시 루트 0개 | 실제 mouse-up 저장과 독립 Visual Reviewer는 포그라운드 환경 의존으로 미검증 |
| 2026-09-22 | `VBoxManage.exe`·`docs/TODO.md`·`docs/VERIFICATION.md` | VDE-017·VDE-021 실행 중 VM 환경 점검 | 실행 중 VM 가드와 Guest Control E2E를 수행할 실제 실행 VM·Guest Additions·자격 증명 경계가 필요함 | `VBoxManage list runningvms`가 비어 있음을 확인하고 TACS `showvminfo --machinereadable`에서 `VMState=poweroff`를 확인; VM 시작·변경 없이 현재 환경의 실행 중 VM/Guest Control 검증 불가 근거를 기록 | 격리 테스트 VM·Guest Additions·자격 증명 주입 정책 확보 전에는 실행 중 VM 기능을 구현하거나 VM을 시작하지 않음 |
| 2026-09-22 | `docs/TODO.md`·`docs/PROJECT_MAP.md` | E-001~E-004 판단 보류 범위 명시 | 클립보드·프로세스 종료·전역 단축키·화면 캡처는 개인정보·권한·전역 동작 계약 없이 구현하면 사용자 범위를 넘을 수 있음 | 각 신규 기능의 사전 결정 항목을 TODO 판단 목록에 추가하고, 결정 전 백그라운드 수집·강제 종료를 구현하지 않도록 명시 | 사용자 제품 정책과 권한 범위 확정 전에는 기능 구현을 시작하지 않음 |
| 2026-09-22 | `app/build.rs`·`app/resources.rc`·`app/*.exe.manifest`·`docs/TODO.md`·`docs/VERIFICATION.md` | G-004 UAC 매니페스트 정리 완료 | `build.rs`는 `/MANIFEST:NO`만 지정하고 세 파일을 참조하지 않음 | 죽은 `resources.rc`와 두 UAC 매니페스트를 삭제하고, 자동 승격 없이 사용자가 관리자 권한으로 직접 실행하는 정책을 `docs/README.md`에 명시 | 권한 정책을 바꾸려면 gpui 매니페스트 충돌·배포 검증을 별도 작업으로 선행 |
