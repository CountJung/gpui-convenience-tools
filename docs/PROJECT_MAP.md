...
| `app/src/virtual_disk/copy.rs` | **구현됨** — 게스트 경로의 호스트 매핑, 예약 이름·길이·링크 추적 안전 정책 | VDE-009; 청크 복사·충돌 정책은 VDE-010~012 |
...
**최종 측정**: 2026-09-21 · `app/src` 총 43개 파일 · 14,281줄
...
### VirtualBox 도메인 (`app/src/virtual_disk/`) — 2,971줄 / 5파일

| 파일 | 줄 | 책임 |
| --- | ---: | --- |
| `mod.rs` | 439 | VDI·파티션·게스트 파일 항목 모델, 정규화된 게스트 경로, `GuestFileSource`, 플랫폼 비의존 오류 계약 |
...
| `copy.rs` | 329 | 절대 대상 루트 기준 게스트 경로 매핑, Windows 예약 이름·경로 길이 검증, 게스트·호스트 리파스 포인트/심볼릭 링크 추적 차단 |
...
| 2026-09-21 | `virtual_disk/copy.rs`·`virtual_disk/mod.rs` | VDE-009 호스트 경로 안전 매핑 추가 | 게스트 경로를 호스트 복사 대상에 연결하는 안전 경계 없음 | 절대 대상 루트, Windows 예약 이름·금지 문자·UTF-16 길이 제한, 게스트 리파스 포인트와 기존 호스트 링크 추적 차단, 6개 정책 테스트 | 실제 파일 복사·충돌·속성·오류 알림 연결은 VDE-010~012에서 수행 |
