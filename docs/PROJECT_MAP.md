...
**최종 측정**: 2026-09-21 · `app/src` 총 40개 파일 · 14,717줄
...
| `partition.rs` | 601 | read-only `PartitionSource` 경계, MBR·EBR·protective MBR·GPT 검색, CRC·LBA 범위 검증 |
...
| 2026-09-21 | `virtual_disk/partition.rs` | VDE-006 MBR/GPT 파서 추가 | 파티션 검색 구현 없음 | 601줄의 MBR·EBR·GPT CRC·범위 검증과 `PartitionSource` 어댑터 | NTFS 파일시스템 판정은 VDE-007~008에서 연결 |
