...
- [ ] VDE-014 | 90% | 탐색기 목록에서 숨김/시스템 항목을 기본 표시하고 폴더 진입·상위 이동·다중 선택·파일 정보 표시 구현 — 목록 행 선택·Ctrl/Shift 다중 선택·폴더 더블클릭·상위 이동·파일 종류/속성/크기와 실제 VBox VDI 숨김/시스템 행 확인 완료; 최신 1200×1000 릴리스 캡처에서 `$Extend`·`many_subdirs` 폴더와 숨김/시스템 파일 행을 다시 확인했으나 폴더 진입 Click은 포그라운드 안전 차단으로 미검증; 외부 키보드·범위 선택은 후속
