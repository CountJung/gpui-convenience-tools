...
### O-3. 파일 복사 안전성
- [ ] VDE-012 | 90% | 원본 변경·복사 실패·대상 권한 부족·지원하지 않는 파일을 토스트/미니 팝업과 로그에 기록하고 항목별 알림 억제 기능 제공 — 오류 분류·부분 복사·안정 억제 키 도메인, 항목별 상세 로그·미억제 오류 토스트·요약 카드의 억제/재표시 버튼·`AppConfig` 영속화와 실제 VBox 생성 VDI 복사 오류 요약 확인 완료; 실제 버튼 클릭 dispatch는 후속

### O-4. GPUI 탐색기

- [ ] VDE-013 | 80% | `ActivePanel`·`NAV_TOOLS`에 `VirtualBox 디스크 탐색`을 등록하고 원본 VDI 선택·파티션 선택·현재 경로 표시·새로고침 구현 — 네비게이션·경로 입력·read-only VDI/파티션 연결·루트 새로고침과 실제 VBox 생성 VDI 성공 경로 완료
- [ ] VDE-014 | 90% | 탐색기 목록에서 숨김/시스템 항목을 기본 표시하고 폴더 진입·상위 이동·다중 선택·파일 정보 표시 구현 — 목록 행 선택·Ctrl/Shift 다중 선택·폴더 더블클릭·상위 이동·파일 종류/속성/크기와 실제 VBox VDI 숨김/시스템 행 확인 완료; 외부 키보드·범위 선택은 후속
- [ ] VDE-015 | 85% | 선택 항목 복사, 대상 폴더 선택, 진행률·중지·완료 요약을 단일 연속 작업형 `scroll_pane`에 배치 — read-only VDI 재연결 백그라운드 복사, 대상 폴더 입력/선택, 진행 이벤트·중지 요청·완료 요약과 실제 VBox VDI 복사 완료; 대용량 폴더 중지 세분화는 후속
...
- [ ] VDE-017 | 85% | UI는 원본 VDI를 쓰지 않는다는 경고, VM 실행 중 직접 접근 불가 상태, 파티션/파일시스템 미지원 상태를 명확히 표시 — 독립 안전 경계 카드, 실행 중 VM·잠금 VDI 전용 오류 안내, NTFS 3.1 외 파일시스템·미확인 파티션 안내와 GPUI 렌더 회귀 테스트·920/1000/1280px 캡처 완료; 실제 실행 중 VM·실제 미지원 이미지 E2E는 후속
...
- `VDE-019`: 실제 VBox 생성 VDI의 파일 행·호스트 복사 대상은 확인했지만, 하네스의
  foreground 입력 제한 때문에 외부 키보드 E2E와 독립 Visual Reviewer 확인은 남아 있다.
