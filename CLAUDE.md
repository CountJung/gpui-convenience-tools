...
하단 내용이 잘릴 수 있다. 너비만 채울 때는 `w_full`을 사용한다.
**가로는 `scroll_pane`이 `overflow_x_hidden`으로 잠근다** — 세로만 열어 두면 컨텐츠가
뷰포트 폭 대신 자기 내용 폭으로 잡혀 섹션마다 오른쪽 끝이 달라진다. 상세 규칙은
copilot-instructions의 「스크롤 컨텐츠 높이 규칙」·「스크롤 컨텐츠 너비 규칙」 절이 정본이다.
...
[sync/mod.rs](app/src/sync/mod.rs)는 UI에 의존하지 않는 순수 로직이라 단위 테스트가 가능하다.
...
**이어서 동기화**: `SyncJob`의 `last_run_unix`·`resume_cursor`는 **엔진 계층이 소유하고 UI는
쓰지 않는다.** UI가 자기 스냅샷을 통째로 저장하므로, `persist_sync_jobs_config`가
`config::carry_over_engine_progress`로 디스크 값을 되살리지 않으면 사용자가 설정을 만질
때마다 실행 위치가 날아간다. 백그라운드는 시작할 때 `last_run_unix`로 주기를 이어받고
(그래서 재시작 직후 전부 실행되지 않는다), 실행 중 5초마다 위치를 남기며, **완주하면 비운다.**
엔진은 커서가 성립하도록 `read_dir` 결과를 이름순으로 정렬한다.
건너뛴 항목도 `seen_names`에 넣어야 **미러 삭제가 확인하지 않은 파일을 지우지 않는다.**

**전역 스위치**: `config.sync_enabled`(사이드바)는 자동 실행만 막는다. 수동 '지금 동기화'는
스위치와 무관하게 실행된다 — 누른 버튼이 아무 반응 없이 무시되면 고장으로 보이기 때문이다.
