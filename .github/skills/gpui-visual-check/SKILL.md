...
- 파일명·경로처럼 핵심 식별 정보에 남은 폭을 우선 배정하고, 종류·속성·크기 같은 보조
  컬럼은 읽을 수 있는 최소 폭을 먼저 확보한다. 보조 컬럼의 고정 폭 때문에 파일명 컬럼이
  지나치게 줄어들면 레이아웃 실패로 판정한다.
- 헤더와 행의 동일 컬럼이 같은 폭 계산을 사용하는지 `debug_bounds`로 확인한다. 고정 열은
  `min_w`·`flex_shrink_0`·`whitespace_nowrap`·`overflow_hidden`을 함께 검사하고, 핵심
  식별자 열에는 최소 폭이 있는지 확인한다.
- 스플리터나 컬럼 폭을 사용자가 조정할 수 있으면 드래그/조절 입력 전후 bounds와 상태를
  확인하고, 설정 저장 후 재시작했을 때 같은 폭이 복원되는지 설정 round-trip 테스트를
  추가한다. 폭 저장 없이 조절만 되는 UI는 완료로 기록하지 않는다.
- 최소 지원 폭·기본 폭·넓은 폭에서 트리/목록 경계와 핵심 컬럼을 캡처한다. 단일 창 크기에서
  정상으로 보이는 것만으로 폭 회귀를 통과시키지 않는다.
